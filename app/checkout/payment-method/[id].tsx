...
export default function PaymentMethod() {
  ...
  return (
    <SafeAreaView className={`flex-1 justify-between ${isDark ? "bg-[#1a1c1d]" : "bg-white"}`} edges={["top", "left", "right", "bottom"]}>
      <ScrollView>
        {/* Header */}
      ...
      <View className={`px-4 py-3 ${isDark ? "bg-[#1a1c1d]" : "bg-white"}`}>
        <TouchableOpacity
          ...
          accessibilityLabel={isProcessing ? "Processing" : "Proceed"}
        >
          {isProcessing && <ActivityIndicator size="small" color="#ffffff" style={{ marginRight: 8 }} />}
