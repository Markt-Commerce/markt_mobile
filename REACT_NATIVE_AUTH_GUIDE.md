...
### React Native and cookies

React Native does not have a browser cookie jar. Whether cookies “just work” depends on the HTTP stack:

- **Expo / React Native Fetch:** On iOS and Android, `credentials: 'include'` uses the native stack, which may store and send cookies for the same origin. Test on a real device: login, then call a protected endpoint (e.g. `/users/profile`) and confirm the cookie is sent.
- **Axios with `withCredentials: true`:** Same idea; behavior is platform-dependent.
- **If cookies are not sent** after login (e.g. 401 on the next request), use a cookie manager so the app can store and attach the `markt_session` cookie:
  - [react-native-cookies](https://github.com/joeferraro/react-native-cookies) or similar, or
  - A backend change to also return a **session token** in the login response and accept `Authorization: Bearer <token>` (then the app stores the token in AsyncStorage and sends it in headers). The current backend is cookie-only.

**Summary:** Prefer one shared API client (Axios or Fetch) with `withCredentials: true` / `credentials: 'include'` for every request so any in-memory cookies are reused. If your tests show cookies not persisting, add a cookie library or ask the backend to support Bearer tokens.
...
// Response interceptor: 401 = redirect to login; 403 = do NOT redirect (wrong role / permission)
apiClient.interceptors.response.use(
  ...
  async (error) => {
    if (error.response?.status === 401) {
      // Not logged in or session expired → clear state and redirect to login
      await AsyncStorage.removeItem('user_session');
      // navigation.navigate('Login');
    }
    // 403 = logged in but not allowed (e.g. seller-only action while in buyer mode).
    // Do not clear session or redirect to login; let the screen show "Switch to seller" or similar.
    return Promise.reject(error);
  }
);
...
### 3. Error Handling: 401 vs 403 (do not treat the same)
- **401 Unauthorized:** User is not logged in (session missing or expired). **Redirect to login** and clear local user state.
- **403 Forbidden:** User is logged in but not allowed (e.g. wrong role: they're in "buyer" mode but hit a seller-only endpoint). **Do not redirect to login.** Show an in-app message; if the message is about seller/buyer role, offer "Switch to seller" (or "Create seller account") so they can fix it without re-logging in.

Redirecting to login on 403 would be wrong: the user is already authenticated; they may just need to switch role. See **docs/FRONTEND_ROLES_AND_SWITCHING.md** for roles, switching, and 401/403 handling in detail.
...
### 5. Account Types and Role Switching
- Users can have **buyer only**, **seller only**, or **both**. They can add the other account later via `POST /users/create-buyer` or `POST /users/create-seller`.
- **Current role** (`current_role`: `"buyer"` or `"seller"`) is returned in login and profile. Use it to show the right UI (e.g. buyer home vs seller dashboard).
- **Switch role:** `POST /users/switch-role` (no body). Allowed only when the user has both accounts. Response includes `previous_role`, `current_role`, `message`. Update local state and optionally refresh profile; no need to log out.
- Full API and UX details: **docs/FRONTEND_ROLES_AND_SWITCHING.md**.
